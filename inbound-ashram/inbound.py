...
from v1.api.twilio_audio_interface import TwilioAudioInterface
from starlette.websockets import WebSocketDisconnect
from fastapi import APIRouter
from callendar.src.core.config import settings
...
async def handle_incoming_call(request: Request):
    ...
    connect = Connect()
    connect.stream(url=f"wss://{settings.BACKEND_HOSTNAME}/twilio/media-stream-eleven")
    response.append(connect)
    return HTMLResponse(content=str(response), media_type="application/xml")

@router.websocket("/twilio/media-stream-eleven")
