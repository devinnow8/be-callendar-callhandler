...
import pytz
from src.utils.encryption_utils import decrypt_data, encrypt_data
import json
from src.services.twilio import get_twilio_service
from src.core.config import settings
import uuid 
import os
from src.services.plivo import PlivoService
import supabase
...
class OrgService(SupabaseService):
    ...
    def update_organisation(self, id, data):
    ...
    async def get_organisation_subscription_details(self, org_id: UUID):
        # get organisation details
        organisation = self.get_organisation_by_id(org_id)
        if not organisation:
            raise HTTPException(status_code=400, detail="Organisation not found")

        response = {
            "org_id": org_id,
            "org_type": organisation.get("type", None),
            "calls_consumed": 0,
            "consumed_call_minutes": 0,
            "rented_numbers_count": organisation.get("rented_numbers_count", 0),
            "package": None,
            ...
            "is_unlocked": organisation.get("is_unlocked", False)
        }
        if organisation.get("is_unlocked") == True:
            return response
        
        # get subscription details
        subscription = await StripeService().get_stripe_subscription_by_org_id(org_id)
        if not subscription:
            return response

        response["calls_consumed"] = organisation.get("calls_consumed", 0)
        response["consumed_call_minutes"] = organisation.get("consumed_call_minutes", 0)
        response["rented_numbers_count"] = organisation.get("rented_numbers_count", 0)
        response["package"] = subscription.get("package", None)
        response["total_calls_allowed"] = subscription.get("total_calls_allowed", 0)
        response["total_call_minutes"] = subscription.get("total_call_minutes", 0)
        response["total_phone_numbers"] = subscription.get("total_phone_numbers", 0)
        response["subscription_end_date"] = subscription.get("end_date", None)
        response["subscription_start_date"] = subscription.get("start_date", None)

        # get top up details
        # top_ups = TopUpService().get_active_top_ups_by_org_id(
        #     organisation_id=org_id, created_at=subscription["start_date"]
        # )
        top_ups = TopUpService().get_subscription_top_ups_by_subscription_id(
        ...
        for top_up in top_ups:
            response["total_call_minutes"] += top_up.get("added_minutes", 0)
...
class CampaignService(SupabaseService):
    ...
    async def update_campaign_stop_status(self, campaign_id, data):
        response = self.client.table("campaigns").update(data).eq("id", campaign_id).execute()
        return self.get_object_or_none(response)
...
class OrganisationContactsService(SupabaseService):
    def get_organisation_contact_by_id(self, org_id, fields: str = "*"):
        ...
        return self.get_objects(contacts)  # Return all contacts as a list
    ...
    async def get_active_organisation_number(self, phone_number, fields: str = "*"):
        contacts = self.client.table("organisation_contacts").select(fields).eq("phone_number", phone_number).eq("status", "active").execute()
        return self.get_object_or_none(contacts)

class InboundCampaignService(SupabaseService):
    async def get_inbound_campaign_by_id(self, id, fields: str = "*"):
        campaign = self.client.table("inbound_campaigns").select(fields).eq("id", id).execute()
        return self.get_object_or_none(campaign)
    
    async def update_inbound_campaign(self, id, data):
        response = self.client.table("inbound_campaigns").update(data).eq("id", id).execute()
        return self.get_object_or_none(response)
    
    async def stop_inbound_campaign(self, campaign_id, org_id):
        try:
            # fetch inbound campaign phone numbers
            inbound_campaign_phone_number_service = InboundCampaignPhoneNumberService()
            inbound_campaign_phone_numbers = await inbound_campaign_phone_number_service.get_inbound_campaign_phone_numbers_by_campaign_id(campaign_id, fields="phone_number, status")
            ...
            for phone_number in inbound_campaign_phone_numbers:
                try:
                    ...
                    if organisation_phone_numbers.get(phone_number["phone_number"]) == "twilio":
                        ...
                        response = twilio_service.add_webhook_endpoint_for_inbound_phone_number(
                            phone_number["phone_number"],
                            answer_url = "",
                            hangup_url = ""
                        )
                        ...
                        response = plivo_service.unlink_phone_number_and_application(phone_number=phone_number["phone_number"])
            ...
            await inbound_campaign_phone_number_service.update_inbound_campaign_phone_numbers_status(campaign_id, inactive_phone_numbers, "inactive")
            
            if len(inactive_phone_numbers) !=  len(inbound_campaign_phone_numbers):
                return None
            
            return await self.update_inbound_campaign(campaign_id, {"status": "Stopped"})
...
class InboundCampaignPhoneNumberService(SupabaseService):
    async def get_inbound_campaign_phone_number_map_by_phone_number(self, phone_number, fields: str = "*"):
        response = self.client.table("inbound_campaign_numbers").select(fields).eq("phone_number", phone_number).execute()
        return self.get_object_or_none(response)
    
    async def get_active_inbound_campaign_phone_number_by_phone_number(self, phone_number, fields: str = "*"):
        response = self.client.table("inbound_campaign_numbers").select(fields).eq("phone_number", phone_number).execute()
        return self.get_object_or_none(response)

    async def get_inbound_campaign_phone_numbers_by_campaign_id(
        self, campaign_id, fields: str = "*"
    ):
        response = (
            self.client.table("inbound_campaign_numbers")
            .select(fields)
            .eq("campaign_id", campaign_id)
            .execute()
        )
    ...
    async def update_inbound_campaign_phone_numbers_status(
        self, campaign_id, phone_numbers, status
    ):
        response = (
            self.client.table("inbound_campaign_numbers")
...
class StripeService(SupabaseService):
    ...
    async def get_stripe_subscription_by_org_id(self, org_id: UUID, fields: str = "*"):
        response = self.client.table("subscriptions").select(fields).eq("org_id", org_id).gt("end_date", datetime.utcnow().isoformat()).order("start_date", desc=True).limit(1).execute()
        return self.get_object_or_none(response)


class TopUpService(SupabaseService):
    def get_active_top_ups_by_org_id(
        self, organisation_id: UUID, created_at: str, fields: str = "*"
    ):
        response = (
            self.client.table("top_ups")
            .select(fields)
    ...
    def get_subscription_top_ups_by_subscription_id(
        self, subscription_id: str, fields: str = "*"
    ):
        response = (
            self.client.table("top_ups")
            .select(fields)
...
class OrganisationSecretsService(SupabaseService):
    def get_organisation_secrets(self, organisation_id: UUID, fields: str = "*"):
        response = (
            self.client.table("organisation_secrets")
            .select(fields)
            .eq("organisation_id", organisation_id)
            .execute()
        )
        secrets = self.get_objects(response)
        if secrets and secrets[0].get("creds"):
            for secret in secrets[0].get("creds"):
                secret["creds"] = json.loads(decrypt_data(secret["creds"]))

        return secrets

    def get_organisation_secret_by_type(
        self, organisation_id: UUID, type: str, fields: str = "*"
    ):
        response = (
            self.client.table("organisation_secrets")
            .select(fields)
        ...
        secret = self.get_object_or_none(response)
        if secret and secret.get("creds"):
            secret["creds"] = json.loads(decrypt_data(secret["creds"]))
        return secret

    def get_organisation_secret_by_id(self, id: UUID, fields: str = "*"):
        response = (
            self.client.table("organisation_secrets")
            .select(fields)
            .eq("id", id)
    ...
    def insert_organisation_secret(self, data):
        data["creds"] = encrypt_data(json.dumps(data["creds"]))
        data["type"] = data["type"].strip().lower()
        response = self.client.table("organisation_secrets").insert(data).execute()
        return self.get_object_or_none(response)
